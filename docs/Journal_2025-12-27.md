...
Think about it:
- Token co-occurrence: ✅ Learned easily
- Basic phrases ("the king"): ✅ Learned
- Long-range dependencies: ❌ Stuck!
...
```
Layer 0: Train until 75% frozen → crystallize
Layer 1: Train until 75% frozen → crystallize
Layer 2: ...
```
...
```
Layer 0 alone:    7.08 → 3.76  (-3.32)
After Layer 1:    3.76 → 3.61  (-0.15)  ← tiny!
After Layer 2:    3.61 → 3.59  (-0.02)  ← nothing!
```
...
**Result:** Smooth loss curve. But no freezing → loss 3.5, behind flat crystal.
...
- But NOT actual sequential dependencies like "the king" → "is"
...
Epoch 60: Loss 2.39, 208 neurons  ← ALREADY BELOW 2.55 BASELINE!
```
...
Now:
- Causal attention builds context: "the" + "king" → rich representation
...
```
BEFORE: Token → Neuron Field → Output
        (each position blind to others)

AFTER:  Token → Causal Attention → Neuron Field → Output
...
| Evening | **95% loss reduction** - 2.55 → 0.12 |
...
**Architecture:**
```
Embedding → Causal Self-Attention → Geometric Neuron Field → FFN → Output
...
*"Turns out it just needed glasses."* 🤓

---

## Part 9: Causal Crystal Compiles to C

### The Goal

With our breakthrough 0.12 loss model, we wanted to compile it to our pure C runtime. But the existing compiler and runtime only supported the "blind" architecture - no causal attention.

### Crystal Format v2

Updated the crystal file format to version 2 with causal attention support:

```c
// New header (64 bytes)
typedef struct __attribute__((packed)) {
    uint32_t magic;           // "CRYS"
    uint16_t version;         // 2 = causal support
    uint16_t flags;           // FLAG_CAUSAL = 0x0080
    uint32_t vocab_size;      // 50,257 (full GPT-2)
    uint16_t embed_dim;       // 128
    uint16_t context_len;     // 64
    uint16_t hidden_dim;      // 256
    uint16_t num_neurons;     // 788
    uint16_t num_frozen;      // 567
    uint16_t num_active;      // 221
    uint16_t num_heads;       // 4 (causal attention heads)
    uint16_t num_norms;       // 3 (causal, geo, ffn)
    uint16_t reserved[2];     // padding
    uint32_t offsets[8];      // section offsets
} CrystalHeader;
```

New data sections:
- **Causal attention weights**: Q, K, V, O projections (4 × [D×D] matrices + biases)
- **Three layer norms**: norm_causal, norm_geo, norm_ffn

### Files Created

**causal_crystal_compiler.py:**
- Detects causal attention in PyTorch checkpoint
- Extracts Q/K/V/O projection weights
- Handles both `attention.*` and `geo_attn.*` key naming
- Writes 64-byte v2 header with FLAG_CAUSAL

**causal_crystal_runtime.c:**
- Reads v2 crystal format
- Implements `causal_attention()` with proper masking
- Implements `geometric_attention()` (existing)
- Three-norm architecture: causal → geo → ffn
- Generation loop with temperature sampling

### Compilation Results

```
$ python causal_crystal_compiler.py \
    crystal_v2/runs/causal_crystal_*/best_model.pt \
    causal_shakespeare.crystal

Causal Crystal Compiler v2.0
========================================
Vocab: 50257, Embed: 128, Context: 64
Neurons: 788 (567 frozen, 221 active)
Causal attention: 4 heads

Final .crystal size: 50.58 MB
...
```bash
$ gcc -O3 -o causal_crystal_runtime causal_crystal_runtime.c -lm
$ ./causal_crystal_runtime causal_shakespeare.crystal "ROMEO:" 100 0.8
```

**C runtime output** (decoded from token IDs):
```
First, this; therefore quite humble daggerons.
Sir, for if his gods,
And, nor pity, to steal,
That flatter elseeds
had thou consul-witted actor...
```
...
The C runtime produces Shakespearean text ("Sir", "thou", "Canst", "attend'st", "Fareth"). Minor quality differences from numerical precision, but the core inference works!

### Architecture in C

```c
void generate_token(Crystal* c, int* context, int ctx_len) {
    // 1. Embed tokens + positions
    for (int t = 0; t < ctx_len; t++) {
        embed(c, context[t], t, &hidden[t * D]);
    }

    // 2. Causal self-attention (NEW!)
    causal_attention(c, hidden, ctx_len);
    layer_norm(c->norm_causal_w, c->norm_causal_b, hidden);

    // 3. Geometric attention
    geometric_attention(c, hidden, ctx_len);
    layer_norm(c->norm_geo_w, c->norm_geo_b, hidden);

    // 4. FFN
    ffn_forward(c, hidden, ctx_len);
    layer_norm(c->norm_ffn_w, c->norm_ffn_b, hidden);

    // 5. Output logits
    compute_logits(c, &hidden[(ctx_len-1) * D], logits);
}
```
...
|-----------|------|--------|
| causal_shakespeare.crystal | 50.58 MB | ✅ Working |
| causal_crystal_runtime | ~45 KB | ✅ Compiles |
| Python inference | Perfect | Baseline |
| C inference | Shakespearean | Working |

The breakthrough causal crystal now runs in pure C - portable to any platform!

---

## Part 10: Shrinking the Crystal - 6x Compression

### The Problem

The causal crystal compiled to 50.58 MB - too big! Full GPT-2 vocabulary (50,257 tokens) when Shakespeare only uses a fraction.

### Step 1: Vocabulary Pruning

Only keep tokens that appear in TinyShakespeare:

```bash
python causal_crystal_compiler.py model.pt output.crystal --prune-vocab data/tinyshakespeare.txt
```

**Result:** 50,257 → 11,706 tokens = **12.79 MB** (4x smaller)

### Step 2: Mixed8 Quantization

Remember from earlier: **embeddings are sacred** (must stay float32), but output head can be int8.

The output head is the biggest tensor: V × D = 11,706 × 128 = 1.5M values.

- Float32: 1.5M × 4 bytes = 6 MB
- Int8: 1.5M × 1 byte = 1.5 MB (+ tiny scale/offset)

Updated compiler to store head weights as int8 with scale/offset:

```python
if self.use_mixed8:
    head_data = struct.pack('<ff', scale_w, offset_w)
    head_data += struct.pack('<ff', scale_b, offset_b)
    head_data += head_w_int8.tobytes() + head_b_int8.tobytes()
```

Updated C runtime to dequantize on the fly:
...
|------|------|-------------|
| Full vocab f32 | 50.58 MB | 1x |
| Pruned vocab f32 | 12.79 MB | 4x |
| **Pruned + mixed8** | **8.47 MB** | **6x** |

### Sample Output (8.47 MB model)

```
When kings did it... And I'll prove his hate music,
upon my death's death... ladies at arms and vengeance...
```

Still Shakespearean! The int8 quantization preserves quality because:
- Embeddings stay float32 (the sacred lookup table)
- Head weights are more tolerant to quantization noise
- Per-tensor scale/offset preserves the distribution
...
PyTorch Model (53 MB)
    │
    ├─ Vocabulary pruning (50K → 11K tokens)
    │
    ├─ Mixed8 quantization (f32 embed + int8 head)
    │
    └─► causal_shakespeare.crystal (8.47 MB)
            │
            └─► Pure C runtime (45 KB binary)
                    │
                    └─► "When kings did it..."
```

**6x compression with the same Shakespeare quality!**
...
|------|-----------|
| Morning | Hierarchical experiments all failed |
| Afternoon | Preserved baseline, spotted blindness bug |
| Afternoon | Added causal self-attention |
| Evening | **95% loss reduction** - 2.55 → 0.12 |
| Night | Compiled causal crystal to C runtime |
| Night | **6x compression** - 50 MB → 8.5 MB |

---

*"The model learned to see."*
*"Then we taught C to see the same way."*
*"8.5 MB of crystallized Shakespeare, runs anywhere."*

---

*"Embeddings are sacred. Everything else can be compressed."*
*"We knew this from the morning. Now the causal crystal knows it too."*
